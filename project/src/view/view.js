...
import { GlobalFooter, GlobalHeader, Preloader} from '@/component/component';
...
const ViewComponent = React.createClass({
  ...
  render() {
    return (
      <div data-route={ this.state.route }>
        <GlobalHeader/>
        <CSSTransitionGroup component='main' transitionName='route' transitionEnterTimeout={ 500 } transitionLeaveTimeout={ 500 } className={ this.state.routeName }>
...
class View {
  constructor() {
    return <Route path='/' component={ ViewComponent } onEnter={ State.enter } onChange={ State.change } onLeave={ State.leave }>
             { Index }
             { About }
             { Work }
           </Route>;
